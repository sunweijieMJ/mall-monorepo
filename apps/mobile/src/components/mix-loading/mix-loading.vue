<template>
  <view class="mix-loading-content">
    <view class="mix-loading-wrapper">
      <image
        class="mix-loading-icon"
        src="data:image/png;base64,"
      ></image>
    </view>
  </view>
</template>

<script setup lang="ts">
/**
 * mix-loading 加载动画组件
 * 显示一个旋转的加载图标
 */

export interface MixLoadingProps {
  /** 距离顶部距离，单位upx */
  top?: number;
}

const props = withDefaults(defineProps<MixLoadingProps>(), {
  top: 0,
});
</script>

<style scoped>
.mix-loading-content {
  display: flex;
  position: absolute;
  top: 0;
  left: 0;
  align-items: center;
  justify-content: center;
  width: 100%;
  height: 100%;
  background-color: transparent;
}

.mix-loading-wrapper {
  display: flex;
  align-items: center;
  justify-content: center;
  animation: loading 0.5s ease-in infinite both alternate;
}

.mix-loading-icon {
  width: 80upx;
  height: 80upx;
  transition: 0.3s;
}

@keyframes loading {
  0% {
    transform: translateY(-20upx) scaleX(1);
  }

  100% {
    transform: translateY(4upx) scaleX(1.3);
  }
}
</style>
