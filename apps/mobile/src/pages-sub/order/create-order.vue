<template>
  <view>
    <!-- 地址 -->
    <navigator
      url="/pages-sub/address/address?source=1"
...
const loadData = async () => {
  try {
    const data = await orderStore.generateConfirmOrder({
    ...
    Object.assign(calcAmount, data?.calcAmount || {});
    integrationConsumeSetting.value = data?.integrationConsumeSetting || {};
    memberIntegration.value = data?.memberIntegration || 0;
  } catch (error) {
