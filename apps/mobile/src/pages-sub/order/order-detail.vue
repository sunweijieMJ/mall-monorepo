...
const loadData = async () => {
  ...
  try {
    const data = await orderStore.fetchDetail(orderId.value);
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        ...
        try {
          await orderStore.cancelOrder(orderIdParam);
          uni.hideLoading();
          loadData();
        } catch (error) {
...
const payOrder = (orderIdParam: number) => {
  uni.redirectTo({
    url: `/pages-sub/order/pay?orderId=${orderIdParam}`,
  });
};
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        uni.showLoading({ title: '请稍后' });
        try {
          await orderStore.confirmReceive(orderIdParam);
          uni.hideLoading();
          loadData();
        } catch (error) {
...
const setOrderStatus = (status: number) => {
  const statusMap: Record<number, OrderStatus> = {
    0: { text: '等待付款', image: '/static/icons/icon_wait.png' },
    1: { text: '等待发货', image: '/static/icons/icon_deliver.png' },
    2: { text: '等待收货', image: '/static/icons/icon_receive.png' },
    3: { text: '交易完成', image: '/static/icons/icon_finish.png' },
    4: { text: '交易关闭', image: '/static/icons/icon_close.png' },
  };
