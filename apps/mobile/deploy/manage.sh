...
# 配置变量
CONTAINER_NAME="mall-mobile-web"
IMAGE_NAME="mall-mobile-web:local"
PORT="${PORT:-9092}"
NGINX_PORT=80
...
# 加载镜像（从 tar 文件）
load_image() {
    local tar_file=$(find . -name "mall-mobile-web*.tar" -type f | head -1)
    ...
    if [ -n "$tar_file" ]; then
        ...
        # 获取加载的镜像名称并重新标记
        local loaded_image=$(docker images --format '{{.Repository}}:{{.Tag}}' | grep 'mall-mobile-web' | head -1)
...
build_image() {
    ...
    if [ ! -d "dist/build/h5" ]; then
        log_error "dist/build/h5 directory not found. Please run 'pnpm build:h5' first."
        exit 1
    fi
