...
const {
  ...
  defaultListQuery,
  (q) => homeProductStore.getNewList(q),
  computed(() => homeProductStore.newList),
  computed(() => homeProductStore.newTotal),
);
...
// 推荐状态切换
const handleRecommendStatusChange = async (row: HomeNewProductVo) => {
...
const { handleDelete } = useDeleteConfirm(
  '推荐',
  (id: number) => homeProductStore.batchDeleteNew([id]),
  getList,
);
...
const handleEditSort = (row: HomeNewProductVo) => {
...
const handleSortConfirm = async (sort: number) => {
  try {
    await homeProductStore.updateNewSort(currentSortItem.value.id, { sort });
