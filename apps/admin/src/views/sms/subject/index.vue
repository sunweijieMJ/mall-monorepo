...
import SelectSubjectDialog from './components/SelectSubjectDialog.vue';
...
import { useHomeSubjectStore } from '@/store';
...
const {
  ...
  defaultListQuery,
  (q) => homeSubjectStore.getList(q),
  computed(() => homeSubjectStore.list),
  computed(() => homeSubjectStore.total),
);
...
// 推荐状态切换
const handleRecommendStatusChange = async (row: HomeSubjectVo) => {
...
const { handleDelete } = useDeleteConfirm(
  '推荐',
  (id: number) => homeSubjectStore.batchDelete([id]),
  getList,
);
...
const handleSelectConfirm = async (subjects: SubjectVo[]) => {
  try {
    const selectSubjects = subjects.map((item) => ({
      subjectId: item.id!,
...
const handleEditSort = (row: HomeSubjectVo) => {
...
const handleSortConfirm = async (sort: number) => {
  try {
    await homeSubjectStore.updateSort(currentSortItem.value.id, { sort });
