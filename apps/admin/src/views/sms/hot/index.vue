...
import type { HomeRecommendProductVo, ProductVo } from '@/api';
...
const list = computed(() => homeProductStore.hotList);
const total = computed(() => homeProductStore.hotTotal);
const multipleSelection = ref<HomeRecommendProductVo[]>([]);
...
const handleSelectionChange = (val: HomeRecommendProductVo[]) => {
...
const handleRecommendStatusChange = async (
  ...
  try {
    ...
    await homeProductStore.updateHotStatus({
      ids: [row.id!],
...
const handleDelete = async (_index: number, row: HomeRecommendProductVo) => {
  try {
    ...
    await homeProductStore.batchDeleteHot([row.id!]);
...
const handleBatchOperate = async () => {
  ...
  try {
    if (operateType.value === 0) {
      ...
      await homeProductStore.batchDeleteHot(ids);
      ElMessage.success('删除成功');
      await getList();
    } else {
...
const handleSelectDialogConfirm = async () => {
  ...
  try {
    ...
    await homeProductStore.batchCreateHot(selectProducts);
...
const handleEditSort = (_index: number, row: HomeRecommendProductVo) => {
...
const handleUpdateSort = async () => {
  try {
    ...
    await homeProductStore.updateHotSort(sortDialogData.id!, {
...
const getList = async () => {
  try {
    await homeProductStore.getHotList(listQuery);
  } catch (error) {
