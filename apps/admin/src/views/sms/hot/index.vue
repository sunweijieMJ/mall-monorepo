...
import { useHomeProductStore } from '@/store';
...
const {
  ...
  defaultListQuery,
  (q) => homeProductStore.getHotList(q),
  computed(() => homeProductStore.hotList),
  computed(() => homeProductStore.hotTotal),
);
...
// 推荐状态切换
const handleRecommendStatusChange = async (row: HomeRecommendProductVo) => {
...
const { handleDelete } = useDeleteConfirm(
  '推荐',
  (id: number) => homeProductStore.batchDeleteHot([id]),
  getList,
);
...
const handleSelectConfirm = async (products: ProductVo[]) => {
  try {
...
const handleEditSort = (row: HomeRecommendProductVo) => {
...
const handleSortConfirm = async (sort: number) => {
  try {
    await homeProductStore.updateHotSort(currentSortItem.value.id, { sort });
