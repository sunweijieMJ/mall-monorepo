...
import { HotProductService, ProductService } from '@/api/modules';
import type { HotProduct, Product } from '@/interface';

const hotProductTableRef = ref<InstanceType<typeof ElTable>>();
...
const list = ref<HotProduct[]>([]);
const total = ref(0);
const multipleSelection = ref<HotProduct[]>([]);
...
const handleSelectionChange = (val: HotProduct[]) => {
...
const handleRecommendStatusChange = async (_index: number, row: HotProduct) => {
  try {
    ...
    await HotProductService.updateRecommendStatus({
      ids: [row.id!],
...
const handleDelete = async (_index: number, row: HotProduct) => {
  try {
    ...
    await HotProductService.deleteHotProduct([row.id!]);
...
const handleBatchOperate = async () => {
  ...
  try {
    if (operateType.value === 0) {
      ...
      await HotProductService.deleteHotProduct(ids);
      ElMessage.success('删除成功');
      await getList();
    } else {
...
const handleSelectDialogConfirm = async () => {
  ...
  try {
    ...
    await HotProductService.createHotProduct(selectProducts);
...
const handleEditSort = (_index: number, row: HotProduct) => {
...
const handleUpdateSort = async () => {
  try {
    ...
    await HotProductService.updateHotProductSort(sortDialogData.id!, {
...
const getList = async () => {
  ...
  try {
    const response = await HotProductService.fetchList(listQuery);
